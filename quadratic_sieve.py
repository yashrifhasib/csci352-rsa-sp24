...
def create_offsets(modulus: int, prime: int) -> list[int]:
...
def reduce_sieve(sieve: list[int], modulus: int, factorBase: list[int]) -> None:
...
def get_relaitons(start_number: int, end_number: int, composite: int, factorBase: list[int]):
    ...
    reduce_sieve(sieve, composite, factorBase)
...
if __name__ == "__main__":
    ...
    start_number = sqrt(N) + 300
    end_number = start_number + 10000
